import { subject, onDestroy, listenOnEventGlobal, _e, _$, createReaseApp } from 'rease';
...
var tsx_root = function tsx_root(props) {
  ...
  var $ratio = subject(getRatio());
  onDestroy(listenOnEventGlobal(WINDOW, 'resize', debounce(function () {
    $ratio.set(getRatio()), resize();
  }, 50)));
  _e("div", {
    class: /*r2.$*/_$([$position], function (_$0) {
    ...
    class: 'cnly-main',
    style: /*r2.$*/_$([$position], function (_$0) {
      return {
    ...
    class: 'cnly-resizer',
    style: /*r2.$*/_$([$ratio], function (_$0) {
      return "transform:scale(" + _$0[0] + ");";
    })
  }, [onPan(function (e, ctx) {
    var delta = e.delta;
    var innerWidth = WINDOW.innerWidth,
      innerHeight = WINDOW.innerHeight;
...
var consolly = function consolly(_temp) {
  ...
  var run = function run() {
    style();
    createReaseApp(document.body, tsx_root, [{
      setup: setup,
      queue: queue,
      count: count,
      timer: timer,
  ...
  if (document.body) run();else onDestroy(listenOnEventGlobal(window, 'load', run));
  return result;
};
export { consolly as default };
